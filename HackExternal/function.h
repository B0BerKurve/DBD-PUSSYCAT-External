...
struct FString : private TArray<wchar_t>
{
	...
	std::string ToString() const
	{
		...
		WideCharToMultiByte(CP_UTF8, 0, ws.c_str(), -1, &str[0], bufferSize, nullptr, nullptr);
...
{
	float fLastSubmitTime = read<float>(mesh + GameOffset.offset_last_render_time);
	float fLastRenderTimeOnScreen = read<float>(mesh + GameOffset.offset_last_submit_time);
...
static std::string GetNameById(uint32_t actor_id)
{
	...
	uint64_t GNameTable = GameVars.dwProcess_Base + GameOffset.offset_g_names;
	...
	if (sLength < 128)
	{
		read_array((TableLocationAddress + 6), pNameBuffer, sLength);
